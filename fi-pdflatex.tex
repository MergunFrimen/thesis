...
\parencite{racek2020acc2}

ChargeFW2 is a program for calculating partial atomic charges of molecules. It uses a empirical methods to calculate the charges. It is available as a stand-alone command line program and in a web-based version in ACC2.

\subsection{AlphaCharges}
\label{subsection:alphacharges}

AlphaCharges ($\alpha\text{Charges}$) is a web application for calculating partial atomic charges of molecules. 

\section{GEMMI}
\label{section:gemmi}

GEMMI is a cross-platform library that provides various tools useful in the field of structural biology. One of these tools is a parser for the mmCIF format, which is used to read and write mmCIF files. The library is written in C++ and has Python bindings. \parencite{wojdyr2022gemmi}
...
As can be seen in Table \ref{table:chargefw2_output_formats}, ChargeFW2 already outputs a mmCIF file. However, the format of this mmCIF file stores the partial atomic charges in a custom \texttt{\_atom\_site.partial\_charge} item, and therefore, it is not compatible with the Mol* viewer, which expects the charges to be stored in the custom mmCIF categories described in Section \ref{section:custom_mmcif_categories}.
...
Figure \ref{fig:result_site} shows the result site for the ACC2 application with the Mol* viewer. The Mol* viewer is configured to load the mmCIF file generated by the backend. The site also contains a drop-down menu, which allows the user to chose which charge set to visualize.

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=\textwidth]{figures/result_site.png}
  \end{center}
...
As described in \ref{subsection:alphacharges}, AlphaCharges ($\alpha\text{Charges}$) is another web application for calculating partial atomic charges. The application shares the architecture with ACC2 on the backend and the frontend. The calculation of the partial atomic charges is done by the SQe+ method.jpo

\section{Viewer extension}

Most of the features required by the AlphaCharges application were already implemented in the Mol* viewer for ACC2. However, there were some additional features that needed to be implemented to support the AlphaCharges application. These features include:

\begin{itemize}
  \item \textbf{AlphaFold coloring}: The application allows the user to color the structure based on the AlphaFold confidence score.
  \item \textbf{Focus on atoms}: The application does not work for some structures. In such cases, the calculation fails and the user is presented with an error message. The user is also able to select the atoms that caused the error and focus on them.
\end{itemize}

\section{Mol* viewer integration}

\section{Wrong structure}
